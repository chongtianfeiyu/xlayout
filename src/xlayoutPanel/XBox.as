package xlayoutPanel 
{
	...
	public class XBox extends Sprite
	{
		...
		public function XBox(p:DisplayObjectContainer,width:Number, height:Number, color:uint=0x000000, borderColor:uint=0x00ff00)
		{
		...
		public static function draw(ob:DisplayObjectContainer):void
		{
			if(!container){
			...
			if(w<=0 || h<=0) return;
			var box:XBox = dic[ob] as XBox;
			var needResize:Boolean = true;
			if(!box){
				needResize = false;
				box = new XBox(container,w,h);
				dic[ob] = box; 
			}
			helpPoint.x = ob.x;
			helpPoint.y = ob.y;
		...
		public static function remove(ob:DisplayObjectContainer):void
		{
			var box:XBox = dic[ob] as XBox;
			if(box){
				dic[ob] = null;
		...
		public static var lastSelObject:DisplayObjectContainer;
		public static function sel(ob:DisplayObjectContainer,yes:Boolean):void
		{
			var box:XBox = dic[ob] as XBox;
			if(box){
				if(!box.hasSel && yes){
